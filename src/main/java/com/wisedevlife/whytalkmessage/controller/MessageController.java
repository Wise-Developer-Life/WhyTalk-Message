...
public class MessageController {
    ...
    @Operation(
            summary =
                    "Get paged messages from a chat room and sorted by created time in descending"
                            + " order")
    ...
    public ResponseEntity<ReturnResponse<ScrollResponse<MessageResponse>>> getAllMessages(
            ...
            @Parameter(description = "offsetId is the id of the anchor message")
                    @RequestParam("lastMessageId")
                    long lastMessageId,
            @Parameter(description = "limit is the number of messages to be returned")
                    @RequestParam("limit")
                    int limit) {
        Page<Message> messages =
                messageService.getPagedMessagesByChatRoomId(chatRoomId, lastMessageId, limit);
        ...
        return ResponseHandler.success(
                ScrollResponse.of(data, lastMessageId, limit, totalElements));
    }
